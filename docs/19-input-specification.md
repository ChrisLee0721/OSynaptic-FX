...
This document defines the input specifications for `OSynapptic-FX` at different calling levels, focusing on answering: who provides transmission rules and which fields are required.
...
On the `OSynapptic-FX` side, this is equivalent to `osfx_glue_encode_sensor_auto(...)` (single sensor) or multi-sensor wrapper.
...
`OSynapptic-FX` supports driving compile-time switches using basic configuration files:

- Configuration source: `OSynapptic-FX/Config.json`
- Generated header: `OSynapptic-FX/include/osfx_build_config.h`
